...
Now we can calculate two means for the level of trust, one for "cable network", the other for "print media". We can compare the difference between the two means and see if it supports our first hypothesis. We can look further by calculating the means of respective age groups from both sub-samples and see if the difference in mean is the largest for young citizens, which in turn help us examine the second hypothesis.

Potential shortcomings: 1) It may be the case that citizens consume political news coverage from multiple/mixed sources, though they would have a preference. And thus, we only know the factual outcome by assigning them into sub-samples. 2) The level of trust may depend on a variety of factors, and there may also be confounder(s) affecting both - people who don't have cable services are systematically different from those have, which restricts their way of consuming political news coverage.
...
A.  A sample of educational funding levels for every country in Europe for each of the last 100 years.
    ...
    The sampling variability tends to be huge, given that we are looking at every European country over a time period of 100 years. Potential bias may include: 1) Over the last 100 years, a number of historical events occurred in Europe, particularly the World War and the Cold War. They both tend to be confounding variables that influence both democratic governance and education funding level. 2) Random sampling error, in which a country with a small population and a small GDP may spend more funding on educational institutions in terms of percentage points of its GDP.
...
B.  A sample of educational funding levels in five randomly picked countries drawn from the global population of countries for each of the last 10 years.
    ...
    The sampling variability may be large or small, depending on the random picks of countries. If all or most of these randomly picked countries are well-developed democratic countries with good governance, the variability could be small. Potential bias may include: Random sampling error, in which the sample statistics differ by chance from the population parameter. This leads to possible huge or small variability depending on the countries picked.
...
C.  A survey of educational administrators that yields responses that allow for a comparison of educational funding levels in Russia and the United States for the six-year period from 2010 to 2015.
    ...
    The sampling variability may be smaller given that both Russia and the US were politically stable in general with relatively good domestic governance during the six-year period. Potential bias may include: 1) Selection bias where educational administrators in different regions may not have an equal chance to be surveyed. 2) Response bias where some educational administrators are more likely to be measured.

D.  A sample of 75 randomly selected countries' educational funding levels for each of the past 25 years.

    The sampling variability may again be either huge or small depending on the countries selected, but the extent may not be that significant in Scenario B since we have a larger sample (75 to 5) and a longer measuring time period. Potential bias may include: Random sampling error where we still have the chances of running into sample statistics differing by chance, though the extent may be smaller with a larger sample size.
